            ...
            if frameNr < n_frames:
                if frameNr % sampling_rate == 0:
                    resized_frame = cv2.resize(
                        frame, (0, 0), fx=downsampling_factor, fy=downsampling_factor
                    )
                    cv2.imwrite(
...
parser.add_argument(
    "--img_path", type=str, default="imgs", help="Output path to store images"
)
parser.add_argument(
    "--sampling_rate", type=int, default=10, help="Temporal Sampling Rate"
)
parser.add_argument(
    "--downsampling_factor", type=float, default=0.3, help="Spatial Downsampling Factor"
)
